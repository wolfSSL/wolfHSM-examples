...
LD_LIBRARY_PATH="$WOLFSSL_DIR/.libs:$WOLFHSM_DIR/.libs" ldd ./$(basename "$SERVER_FULL_PATH")

# Start server with debug output
echo "Starting server with libraries from:"
echo "  wolfSSL: $WOLFSSL_DIR/.libs"
echo "  wolfHSM: $WOLFHSM_DIR/.libs"
LD_LIBRARY_PATH="$WOLFSSL_DIR/.libs:$WOLFHSM_DIR/.libs" ./$(basename "$SERVER_FULL_PATH") > server.log 2>&1 &
SERVER_PID=$!
